...
- **Phase 6 — firmware + camera:** `POST /ingest/snapshot` (multipart JPEG), **`device_snapshots`** metadata in Postgres, image bytes in **S3-compatible storage** (use a **Railway Storage bucket** in production), presigned URLs on **`getSite.latestSnapshot`** and **`adminDevice.recentSnapshots`**, latest snapshot on site detail, esp-web-tools install wizard, PlatformIO firmware under [`firmware/aquaponics-node/`](firmware/aquaponics-node/), placeholder [`apps/web/public/firmware/esp8266/firmware.bin`](apps/web/public/firmware/esp8266/firmware.bin) (replace with a real `pio run` build before flashing devices).
