...
import {
  useDeleteSensorCatalogEntryMutate,
  useSensorCatalog,
  useUpdateSensorCatalogEntryMutate,
} from '~/hooks/useAdmin';
import {
...
export function AdminSensorEditPageContent() {
  ...
  const { mutateAsync: updateRow, isPending: isSaving } =
    useUpdateSensorCatalogEntryMutate();
  const { mutateAsync: deleteRow, isPending: isDeleting } =
    useDeleteSensorCatalogEntryMutate();

  const row = useMemo(
    () => catalog?.find((c) => c.key === sensorKey),
    [catalog, sensorKey],
  );
  ...
  useEffect(() => {
    ...
    setIcon(row.icon ?? '');
    setWiringTemplate(normalizeWiringTemplateFromGraphql(row.wiringTemplate));
  }, [row]);
          ...
          <form className='space-y-4' onSubmit={(e) => void onSubmit(e)}>
            ...
            <div className='grid gap-4 sm:grid-cols-2'>
              <div className='space-y-2'>
                ...
                <Input
                  id='pmin'
                  value={physicalMin}
                  onChange={(e) => setPhysicalMin(e.target.value)}
                />
              </div>
              <div className='space-y-2'>
                ...
                <Input
                  id='pmax'
                  value={physicalMax}
                  onChange={(e) => setPhysicalMax(e.target.value)}
                />
              </div>
            </div>
            <AdminSensorIconField id='ic' value={icon} onChange={setIcon} />
            <SensorWiringEditor
              value={wiringTemplate}
              onChange={setWiringTemplate}
            />
            {formError ? (
            ...
            <div className='flex flex-wrap gap-2'>
              <Button type='submit' disabled={isSaving}>
                <ButtonPendingLabel pending={isSaving}>
                  {t('admin.shared.save')}
                </ButtonPendingLabel>
              </Button>
              <Button
