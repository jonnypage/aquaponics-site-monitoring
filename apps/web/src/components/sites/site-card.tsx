...
export function SiteCard({ id, name, status, lastUpdate }: SiteCardProps) {
  ...
  const lastSeen = lastUpdate ? formatRelativeTime(new Date(lastUpdate)) : t("siteCard.noReadingsYet");
      ...
      <Card className="transition-shadow hover:shadow-md">
        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3">
          <div className="space-y-1">
            <CardTitle className="text-lg group-hover:text-primary">{name}</CardTitle>
            <CardDescription className="text-xs">{t("siteCard.idLabel", { id })}</CardDescription>
          </div>
