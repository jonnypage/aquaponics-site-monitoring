...
| `apps/web/src/routes/_authed.admin*.tsx` | **`requireAdmin`** in layout; `/admin` hub + users/sites/devices/sensors CRUD pages |
| `apps/web/src/routes/_authed.tsx`  | Pathless layout route; calls `requireAuth` + renders `DashboardShell` (`showAdminNav` when `ADMIN`). All authenticated pages live under `_authed.*.tsx`. |
